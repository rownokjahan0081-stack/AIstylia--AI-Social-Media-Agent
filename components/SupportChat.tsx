        ...
        <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
            {isOpen && (
                <Card className="w-80 md:w-96 h-[500px] bg-slate-800 shadow-2xl border border-slate-700 flex flex-col mb-4 animate-fade-in-up">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-slate-800/50 relative">
                        ...
                        {!isLoading && messages.length > 0 && (
                            <div className="flex justify-center mt-4 mb-2">
                                <span className="text-xs text-slate-500 bg-slate-900/50 px-3 py-1 rounded-full border border-slate-700/50 animate-pulse">
                                    Ask Anything...
                                </span>
                            </div>
                        )}
                    ...
                    <form onSubmit={handleSend} className="p-3 border-t border-slate-700 bg-slate-900/30 rounded-b-xl flex gap-2">
                        <input 
                            ...
                            className="flex-1 bg-slate-700 border-none rounded-md px-3 py-2 text-sm text-white focus:ring-2 focus:ring-sky-500 outline-none placeholder:text-slate-500"
                        />
                        <button 
                            type="submit"
